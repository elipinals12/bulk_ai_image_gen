...
OUTPUT_DIR = "inputs/aquateak_products"
LOG_FOLDER = "logs"
LOG_FILE = os.path.join(LOG_FOLDER, "scraper_log.txt")
...
def log_skip(error_category, sku, details):
    error_log[error_category].append({'sku': sku, 'details': details})
...
def is_white_background(image_path, sku):
    try:
        ...
        if not is_white:
            log_skip("Background Too Dark", sku, f"{white_ratio*100:.1f}% white (needs {WHITE_PERCENTAGE*100:.1f}%)")
...
def extract_category_structure():
    try:
        ...
        for nav_item in top_level_items:
            ...
            if not links:
                continue
            top_name = links[0].get_text(strip=True)
            if top_name not in ['Bathroom', 'Indoor', 'Outdoor']:
                continue
            structure[top_name] = {}
            for dropdown_ul in nav_item.find_all('ul'):
                for section_item in dropdown_ul.find_all('li', recursive=False):
                    ...
                    for granular_item in sub_ul.find_all('li'):
...
def scrape_category_page(category_url, page=1):
    try:
        ...
        for card in soup.select('.product'):
            ...
            products.append({'name': name_tag.get_text(strip=True), 'url': product_url})
        has_next = soup.select_one('.pagination-item--next:not(.pagination-item--disabled)')
        return products, has_next is not None
    except Exception as e:
...
def scrape_product_details(product):
    ...
    try:
        ...
        log_skip("Page Fetch Error", "UNKNOWN", str(e))
        return None
...
def download_image(product, category_folder, category_path):
    ...
    if sku in downloaded_products:
        log_skip("Duplicate Product", sku, f"Already in: {downloaded_products[sku]}")
        tqdm.write(f"  ⚠ SKIP - {sku} - Duplicate")
        return False
    ...
    try:
        ...
        ext = '.png' if 'png' in content_type.lower() or product['image_url'].lower().endswith('.png') else '.jpg'
...
def main():
    ...
    total_categories = sum(len(g) for t in structure.values() for m in t.values() for g in [m] if isinstance(m, dict))
    ...
    for top_name, mid_level in structure.items():
        for mid_name, granular_level in mid_level.items():
            for granular_name in granular_level.keys():
                Path(os.path.join(OUTPUT_DIR, sanitize_folder_name(top_name), 
                     sanitize_folder_name(mid_name), sanitize_folder_name(granular_name))).mkdir(parents=True, exist_ok=True)
    ...
    with tqdm(total=total_products, desc="Processing", unit="product") as pbar:
        for top_name, mid_level in structure.items():
            for mid_name, granular_level in mid_level.items():
                for granular_name, category_url in granular_level.items():
                    category_path = f"{top_name}/{mid_name}/{granular_name}"
                    folder_path = os.path.join(OUTPUT_DIR, sanitize_folder_name(top_name),
                                               sanitize_folder_name(mid_name), sanitize_folder_name(granular_name))
                    page = 1
                    while True:
                        products, has_next = scrape_category_page(category_url, page)
                        if not products: break
    ...
    write_log_file(total_products, successful, skipped, duplicates)
